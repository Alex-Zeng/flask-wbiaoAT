...
from app.blue.runtest.base.base_funci_info import func_dict
...
class BaseAction:
    ...
    def __init__(self, driver, log, thread_name):
        self.driver = driver
        self._by_type = {'xpath': MobileBy.XPATH, 'id': MobileBy.ID, 'css': MobileBy.CSS_SELECTOR,
        ...
        self.ele_wait_time = int(self.conf.get_common_setting('ele_wait_time'))
        self.screenshot_wait_time = float(self.conf.get_common_setting('screenshot_wait_time'))
        self.case_title = ''
    ...
    def action(self, case_list, data):
        ...
        for v in case_list:
            ...
            # excle 描述的方法,主要用于报告
            excle_action = v['action']
            action = func_dict.get(excle_action)
            find_type = v['type']
            ...
            text = ''
            self.log.info('执行用例{}:{}'.format(case_id, v['step']))
            if find_type or element_loc:
                loc = self._by_type.get(find_type), element_loc
            else:
                loc = MobileBy.XPATH, "空值"
            ...
            try:
                ...
                if input_data != '':
                    text = test_data.get(input_data).popleft()
                    # 保证输出函数可持续使用
                    test_data.get(input_data).append(text)

                output_text = self.__getattribute__(action)(loc,text)
                ...
                if output_data != '':
                    ...
                    # 输处返回参数，给依赖用例使用
                    d = deque()
                    d.appendleft(output_text)
                    test_data[output_data] = d

                allure.attach(str(output_text) if output_text else str(text),
                              '{}: {} --- {} --- {}'.format(case_id, excle_action, element_info, '成功'))
            except Exception as e:
                allure.attach('错误: {} --- {} --- {}'.format(excle_action, element_info, loc),
    ...
    def check_element(self, *args):
        ...
        try:
            ...
            allure.attach(f, '成功找到{}元素，截图保留'.format(args[0]), allure.attachment_type.PNG)
            self.log.info('断言成功')
        except Exception as e:
            self.log.error('断言失败未找到元素{}'.format(args[0]))
            f = self.driver.get_screenshot_as_png()
            allure.attach(f, '未找到{}元素，截图保留'.format(args[0]), allure.attachment_type.PNG)
            raise e
    ...
    def check_activity(self, *args):
        ...
        try:
            ...
            f = self.driver.get_screenshot_as_png()
            allure.attach(f, '预期页面，截图保留'.format(args[0]), allure.attachment_type.PNG)
            self.log.info('是预期页面')
            ...
            f = self.driver.get_screenshot_as_png()
            allure.attach(f, '非预期页面，截图保留'.format(args[0]), allure.attachment_type.PNG)
            raise e
    ...
    def take_screen_shot(self, *args, name='截图',wait_time=None):
        ...
        fq = self.conf.get_filepath('screenShotsDir') + os.sep + day + os.sep + self.thread_name + '_' + self.case_title
        img_type = '.png'
        ...
        if os.path.exists(fq):
            ...
            filename = fq + os.sep + tm + "_" + self.current_case.get(
                'step', 'action') + name + img_type
        time.sleep(wait_time if wait_time else self.screenshot_wait_time)
